 ...
 * @param flags 参�?? [Spanned.SPAN_EXCLUSIVE_EXCLUSIVE]
 *
 * @return 如果[this]不为[Spannable]则将返回�?个新的[SpannableStringBuilder]对象
 */
@JvmOverloads
 ...
 *
 * @return 如果接受者不为[SpannableStringBuilder]则将返回�?个新的[SpannableStringBuilder]对象
 */
 ...
 * 替换匹配的字符串
 *
 * @param oldValue 被替换的字符�?
 * @param ignoreCase 忽略大小�?
 * @param startIndex 从指定的索引�?�?
 * @param replacement 每次匹配到字符串都会调用此函�?, 该函数返回�?�可以是[Spanned]或[Spanned]数组/集合或[CharSequence], 空字符则无效
 * 1. 如果返回null则表示不执行任何操作
 * 2. 返回单个Span则应用效�?, 当然返回Span集合或数组就会应用多个效�?,
 * 3. 返回[Spanned]可以替换字符串同时添加Span效果.
 * 4. 返回[CharSequence]则仅仅是替换字符�?.
 * 5. 并且本函数支持反向引用捕获组, 使用方法等同于RegEx: $捕获组索�?
 * 6. 和[replace]函数不同的是本函数会保留原有[Spanned]的效�?
 *
 * @return
 * 1. 没有匹配任何项返回[this]
